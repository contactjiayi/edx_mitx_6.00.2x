...
def r_squared(y, estimated):
    ...
    """
    sum_squared_errors = sum([(y[i] - estimated[i]) ** 2 for i in range(len(y))])
    mean = sum(y) / len(y)
    variability = sum([(y[j] - mean) ** 2 for j in range(len(y))])
    return 1 - (sum_squared_errors / variability)
...
def evaluate_models_on_training(x, y, models):
    ...
    best = (float('inf'), None, [])
    for model in models:
        y_estimated = pylab.polyval(model, x)
        explained_variability = r_squared(y, y_estimated)
        if explained_variability > best[0]:
            best = (explained_variability, model, y_estimated)

    r_squared_best, model_best, y_best = best

    pylab.plot(x, y, 'bo', label='Data')
    pylab.xlabel('Years')
    pylab.ylabel('Temperature')
    pylab.plot(x, y, 'r-', label='Best Fit')
    pylab.title('Degree: {}\nR^2: {}%'.format(
        str(len(model_best)),
        str(r_squared_best * 100),
    ))
